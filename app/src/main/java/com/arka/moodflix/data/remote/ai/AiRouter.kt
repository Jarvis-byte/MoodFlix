...
class AiRouter @Inject constructor(
    ...
    suspend fun suggest(prompt: String): AppResult<RoutedSuggestions> {
        ...
        for (type in order) {
            ...
            triedAny = true
            Log.d(TAG, "Trying ${type.displayName}")

            when (val result = client.suggest(key, prompt)) {
                is AppResult.Success -> {
                    Log.d(TAG, "${type.displayName} answered with ${result.data.size} picks")
                ...
                is AppResult.Failure -> {
                    ...
                    Log.w(TAG, "${type.displayName} failed: ${result.error.message}, falling back")
                }
            }
        }

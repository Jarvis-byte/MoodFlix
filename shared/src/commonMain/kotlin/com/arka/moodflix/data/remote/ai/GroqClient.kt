...
@Serializable
private data class GroqResponse(val choices: List<Choice> = emptyList()) {
    @Serializable data class Choice(val message: Message? = null)
    @Serializable data class Message(val content: String = "")
}

class GroqClient(private val client: HttpClient) : AiProviderClient {
    ...
    override suspend fun suggest(apiKey: String, prompt: String): AppResult<List<AiSuggestion>> {
        val body = GroqRequest(
            ...
            messages = listOf(
                GroqRequest.Message("system", PromptBuilder.systemInstruction),
                GroqRequest.Message("user", prompt)
            )
        )
        ...
        return try {
            ...
            val text = response.body<GroqResponse>()
